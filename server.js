...
function extractNumber(str) {
  ...
  const properties = {
    ...
    STR: { select: pcData.status?.NP1 ? { name: String(pcData.status.NP1) } : null },
    CON: { select: pcData.status?.NP2 ? { name: String(pcData.status.NP2) } : null },
    POW: { select: pcData.status?.NP3 ? { name: String(pcData.status.NP3) } : null },
    DEX: { select: pcData.status?.NP4 ? { name: String(pcData.status.NP4) } : null },
    APP: { select: pcData.status?.NP5 ? { name: String(pcData.status.NP5) } : null },
    SIZ: { select: pcData.status?.NP6 ? { name: String(pcData.status.NP6) } : null },
    INT: { select: pcData.status?.NP7 ? { name: String(pcData.status.NP7) } : null },
    EDU: { select: pcData.status?.NP8 ? { name: String(pcData.status.NP8) } : null },
