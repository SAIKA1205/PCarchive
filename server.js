...
function extractNumber(str) {
  ...
  const properties = {
    ...
    STR: { select: pcData.NP1? { name: String(pcData.status.NP1) } : null },
    CON: { select: pcData.NP2? { name: String(pcData.status.NP2) } : null },
    POW: { select: pcData.NP3? { name: String(pcData.status.NP3) } : null },
    DEX: { select: pcData.NP4? { name: String(pcData.status.NP4) } : null },
    APP: { select: pcData.NP5? { name: String(pcData.status.NP5) } : null },
    SIZ: { select: pcData.NP6? { name: String(pcData.status.NP6) } : null },
    INT: { select: pcData.NP7? { name: String(pcData.status.NP7) } : null },
    EDU: { select: pcData.NP8? { name: String(pcData.status.NP8) } : null },
