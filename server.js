...
async function updateNotionDatabase(notion, databaseId, pcData, characterId) {
  ...
  const properties = {
    ...
    SAN値: { select: pcData.status?.SAN ? { name: String(pcData.status.SAN) } : null },
    アイデア: { select: pcData.status?.アイデア ? { name: String(pcData.status.アイデア) } : null },
    幸運: { select: pcData.status?.幸運 ? { name: String(pcData.status.幸運) } : null },
    知識: { select: pcData.status?.知識 ? { name: String(pcData.status.知識) } : null },
